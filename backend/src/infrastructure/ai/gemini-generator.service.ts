...
export class GeminiGeneratorService implements IAIGenerator {
  ...
  private readonly SKIP_FILES = ['app.ts', 'main.ts', 'index.ts', 'jest.config.ts'];
  private readonly SKIP_FOLDERS = ['interfaces', 'dto', 'entities', 'migrations', 'node_modules', 'dist', 'coverage', 'types'];
  private readonly SKIP_EXTENSIONS = ['.interface.ts', '.d.ts', '.module.ts', '.entity.ts', '.dto.ts', '.spec.ts', '.test.ts'];
  ...
  async generateTest(
    ...
    this.logger.log(`Starting isolated AI CLI generation for ${sourceFilePath}...`);
    ...
    const tempTestFilePath = fullTestFilePath.replace(/\.(spec|test)\.ts$/, '.verification.test.ts');

    try {
      // 1. Context Awareness (AST Analysis)
      const [sourceCode, availablePackages, dependencyContext] = await Promise.all([
      ...
      const importPath = this.getRelativeImportPath(sourceFilePath, testFilePath);
      const dependencySignatures = this.dependencyAnalyzer.formatContext(dependencyContext);
      const systemInstruction = this.promptBuilder.buildSystemInstruction(targetCoverage);

      let currentError = '';
      const MAX_ATTEMPTS = 3;

      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        this.logger.log(`Attempt ${attempt}/${MAX_ATTEMPTS} for ${sourceFilePath}`);
        ...
        try {
          ...
          try {
            ...
            throw new Error(`Failed to parse AI CLI JSON: ${e.message}\nOutput: ${cliResult.output}`);
          }

          generatedCode = Array.isArray(parsed) 
            ? parsed.map(p => p.candidates?.[0]?.content?.parts?.[0]?.text).join('')
            : parsed.response || parsed.text || parsed.candidates?.[0]?.content?.parts?.[0]?.text;

          if (!generatedCode && parsed.error) {
            throw new Error(`AI CLI returned error: ${parsed.error.message || JSON.stringify(parsed.error)}`);
          }

        } catch (execError) {
          throw new Error(`Isolated AI CLI execution failed: ${execError.message}`);
        } finally {
        ...
        this.logger.debug(`Wrote verification test: ${tempTestFilePath} (${stats.size} bytes)`);

        // Step 2: Validate (TSC first, then Jest + Coverage)
        this.logger.log(`Validating attempt ${attempt}...`);
        const validation = await this.testValidator.validateTest(tempTestFilePath, localRepoPath, targetCoverage);

        if (validation.success) {
          // Success! Move temp file to actual test path and clean up
          await fs.rename(tempTestFilePath, fullTestFilePath);
          this.logger.log(`Successfully generated valid test for ${sourceFilePath} after ${attempt} attempts.`);
          return;
        }

        this.logger.warn(`Attempt ${attempt} failed validation: ${validation.error.substring(0, 500)}`);
        this.logger.debug(`Generated code for attempt ${attempt}:\n${generatedCode}`);
        currentError = validation.error;
      }

      // Cleanup temp file on final failure
      try { await fs.unlink(tempTestFilePath); } catch {}
      
      throw new Error(`Failed to generate a valid test for ${sourceFilePath} after ${MAX_ATTEMPTS} attempts. Last error: ${currentError}`);
