...
export class TestValidatorService {
  ...
  ): Promise<ValidationResultWithCoverage> {
    try {
      ...
      const tscArgs = ['--noEmit', '--project', 'tsconfig.validation.json'];

      this.logger.debug(`Running sandboxed TSC: tsc ${tscArgs.join(' ')}`);
      const tscResult = await this.sandboxExecutor.runSandboxedCommand(
      ...
      const relativeSourceFilePath = path.relative(
        repoPath,
        testFilePath.replace(/\.verification\.test\.ts$/, '.ts'),
      );
...
`;
      ...
      this.logger.debug(
        `Running sandboxed Jest with config: ${path.basename(jestConfigPath)}`,
      );
      ...
      if (!cleanJson.includes('"coverageMap"')) {
        this.logger.warn(
          `Jest failed to produce coverage JSON for ${path.basename(testFilePath)}. Full output: ${jestResult.output.substring(0, 1000)}`,
        );
        if (!jestResult.success) {
          return {
            success: false,
            error: `Jest (Execution) Error:\n${jestResult.output}\n\n${!tscResult.success ? `TSC (Type) Context:\n${tscResult.output}` : ''}`,
            coverage: 0,
          };
        }
      }
      ...
      this.logger.debug(
        `Target coverage for ${path.basename(testFilePath)}: ${targetCoverage}%`,
      );
      ...
      try {
        ...
        const sourcePath = testFilePath.replace(
        ...
        const sourceFileKey = Object.keys(coverageMap).find(
          (key) => key.endsWith(targetRelativePath) || key.endsWith(sourceBase),
        );

        if (!sourceFileKey) {
          this.logger.warn(
            `Could not find coverage for ${sourceBase} in keys: ${Object.keys(coverageMap).join(', ')}`,
          );
          return {
            success: false,
            error: `COVERAGE ERROR: Could not find coverage data for ${sourceBase}. Jest might have failed to collect it.`,
            coverage: 0,
          };
        }
        ...
        if (stats.s && typeof stats.s.pct === 'number') {
          ...
          const covered = Object.values(statementHits).filter(
            (v) => (v as number) > 0,
          ).length;
          pct = total > 0 ? (covered / total) * 100 : 0;
          this.logger.debug(
            `Calculated coverage for ${sourceBase}: ${covered}/${total} statements (${pct.toFixed(2)}%)`,
          );

          if (pct === 0 && total > 0) {
            this.logger.debug(
              `Coverage is 0%. Sample stats.s: ${JSON.stringify(stats.s).substring(0, 200)}`,
            );
          }
        }

        this.logger.log(
          `Enforcing coverage: ${sourceBase} is ${pct.toFixed(2)}% (Target: ${targetCoverage}%)`,
        );

        if (pct < targetCoverage) {
          const uncoveredLines = stats.s
            ? Object.entries(stats.s)
        ...
        return {
          success: false,
          error: `VALIDATION ERROR: Failed to parse coverage results: ${e.message}`,
          coverage: 0,
        };
      }
    } catch (e) {
      return {
        success: false,
        error: `System error during validation: ${e.message}`,
        coverage: 0,
      };
    }
  }
