...
export class ImproveCoverageUseCase {
  ...
  constructor(
    ...
    @Inject(REPOSITORY_HOST_TOKEN) private readonly githubService: IRepositoryHost,
  ...
  async execute(jobId: string): Promise<void> {
    ...
    try {
      ...
      const hasPermissions = await this.githubService.checkPermissions(job.repositoryUrl);
      if (!hasPermissions) {
        throw new Error(`Insufficient permissions to push to ${job.repositoryUrl}. Please check your GITHUB_TOKEN.`);
      }

      // 1. CLONING
      await this.updateJobStatus(job, JobStatus.CLONING);
      localRepoPath = await this.githubService.cloneRepository(job.repositoryUrl);

      // Default branch can vary, so we need to get it from the repository
      const baseBranch = await this.githubService.getDefaultBranch(localRepoPath);
      ...
      const possibleTestPaths = [
        ...
        path.join(localRepoPath, `${job.filePath.replace(/\.tsx?$/, '')}.spec.ts`)
      ];
      ...
      for (const artefact of ['coverage', 'tsconfig.validation.json', 'jest.config.verification.js']) {
        await fs.rm(path.join(localRepoPath, artefact), { recursive: true, force: true }).catch(() => {});
      }
      ...
      if (localRepoPath) {
        await this.githubService.cleanupLocalRepository(localRepoPath).catch((e: any) => {
  ...
  private async updateJobStatus(job: ImprovementJob, status: JobStatus, error?: string, prLink?: string) {
